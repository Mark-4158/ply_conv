...
static ssize_t conv_write(const int sockfd,
    ...
    union {
        const char *s;
        ssize_t len;
    } res = { memchr(msg->msg, '\0', UINT8_MAX) };
    uint8_t len;
    const char *header;

    if (msg->msg_style == PAM_ERROR_MSG) {
        len = 4;
        header = PLY_BOOT_PROTOCOL_REQUEST_TYPE_ERROR "\x2"
                 PLY_BOOT_PROTOCOL_REQUEST_TYPE_SHOW_MESSAGE "\x2";
    } else {
        len = 2;

        if (msg->msg_style == PAM_TEXT_INFO) {
            header = PLY_BOOT_PROTOCOL_REQUEST_TYPE_SHOW_MESSAGE "\x2";
        } else {
            if (msg->msg_style == PAM_PROMPT_ECHO_OFF) {
                header = PLY_BOOT_PROTOCOL_REQUEST_TYPE_PASSWORD "\x2";
            } else if (msg->msg_style == PAM_PROMPT_ECHO_ON) {
                header = PLY_BOOT_PROTOCOL_REQUEST_TYPE_QUESTION "\x2";
            } else {
                warnx("conv_write(): unsupported message style requested");
                return -1;
            }

            while (res.s > msg->msg && res.s[-1] > -1 && !isalnum(res.s[-1])) {
                --res.s;
            }
        }
    }

    if (ply_write(sockfd, header, len)) {
        if ((res.len = res.s - msg->msg) < 0) {
            res.len = UINT8_MAX;
        }
        len = res.len;

        if (ply_write(sockfd, &len, sizeof len) &&
            ply_write(sockfd, msg->msg, res.len)) {
            return res.len;
        }
    }
    warn("conv_write()");
