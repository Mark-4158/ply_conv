...
static ssize_t conv_write(const int sockfd,
    ...
    static_assert(PAM_PROMPT_ECHO_OFF == 1);
    static_assert(PAM_PROMPT_ECHO_ON == 2);
    static_assert(PAM_ERROR_MSG == 3);
    static_assert(PAM_TEXT_INFO == 4);

    if ((unsigned)msg->msg_style > 4u) {
