...
static ssize_t conv_write(const int sockfd,
    ...
    uint_fast8_t len = strnlen(msg->msg, UINT8_MAX - 1);
    uint8_t query[len < sizeof(ssize_t) - 3 ? sizeof(ssize_t) : len + 4u] = { };
    ...
    query[2] = 1;
    while (len) {
        register const char ch = msg->msg[--len];

        if (ch == '!' || ch == '.' || ch == '?' || isalnum(ch)) {
            query[2] += ++len;
            ((char *)memcpy(query + 3, msg->msg, len))[len] = '\0';
            break;
        }
    }
